"""``prometheon verify-validator`` — link a hotkey to a BitFan validator account.

Identical structure to ``verify-miner``; the ``role`` field in the
canonical payload is the only on-wire difference. The default API-token
env var matches the validator naming convention.
"""
...
@click.command(name="verify-validator")
...
@click.option("--platform-instance-id", required=True, help="Configured platform_instance_id.")
@click.option(
    "--chain-network",
...
def verify_validator(
    ...
    """Verify a Bittensor hotkey as a BitFan validator account.

    On success the platform sets ``validator_verified = true`` on the
    linked account and consumes the issued nonce.
    """
    token = read_api_token_or_exit(
        env_var=api_token_env, explicit=api_token, bootstrap_for_role="validator"
    )
    ...
    echo_success(
        f"validator verification accepted (account={nonce.platform_account_id}, "
        f"hotkey={keypair.ss58_address})"
    )
    click.echo(f"platform response: {response}")


__all__ = ["verify_validator"]

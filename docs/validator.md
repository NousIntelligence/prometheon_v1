...
| Bootstrap token (first verify only) | Obtained from the [BitFan portal](https://bitfan.ai/me/prometheon) — click *Get bootstrap token* with role *validator*. The token is one-time, scoped to `identity:verify:validator`, expires after one hour, and is auto-revoked the moment `verify-validator` succeeds. Export it as `PROMETHEON_VALIDATOR_API_TOKEN`. |
...
Each cycle the runner countersigns every sealed day digest whose
`records_hash` matches the records it holds, using the validator hotkey.
The result is appended to `attestations.ndjson` in the state directory: a
...
Full record shape, verification rules, and the manual `ingest attest`
command: [`digest-attestation.md`](./digest-attestation.md).

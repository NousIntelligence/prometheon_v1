...
| Bootstrap token (first verify only) | Obtained from the [BitFan Partner Portal](https://bitfan.ai/me/partner/prometheon) — click *Get bootstrap token* with role *miner*. The token is one-time, scoped to `identity:verify:miner`, expires after one hour, and is auto-revoked the moment `verify-miner` succeeds. Export it as `PROMETHEON_MINER_API_TOKEN`. |
